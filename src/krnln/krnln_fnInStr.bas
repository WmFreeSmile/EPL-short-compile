...
sub krnln_fnInStr cdecl(pRetData as PMDATA_INF,uArgCount as ulong, pArgInf as PMDATA_INF) 
	dim nStart as long
    ...
    if(pArgInf[2].m_dtDataType=_SDT_NULL orelse pArgInf[2].m_int<=1) then
        nStart=0
    else
        nStart=pArgInf[2].m_int-1
    end if
    
    dim pStart as byte ptr=str1+nStart
    
    dim cp as byte ptr=str1
    dim s1 as byte ptr,s2 as byte ptr
