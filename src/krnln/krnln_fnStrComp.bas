...
sub krnln_fnStrComp cdecl(pRetData as PMDATA_INF,uArgCount as ulong, pArgInf as PMDATA_INF) 
    if(pArgInf[2].m_bool) then
        pRetData->m_int=strcmp(pArgInf[0].m_pText,pArgInf[1].m_pText)
        return
    end if
	pRetData->m_int=_stricmp(pArgInf[0].m_pText,pArgInf[1].m_pText)
end sub
